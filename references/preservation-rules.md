...
- OCR is off by default and translates nothing inside an image. With it enabled (`--ocr`, or the GUI checkbox), every image region is recognized, translated a paragraph at a time, and drawn back over the lines it read; the backing covers those line boxes only, never the artwork around them, and it is painted the colour sampled from the paper underneath rather than white, and a translation that will not fit its box even at half the size it was read at is left as pixels and reported.
