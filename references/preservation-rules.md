...
- OCR is off by default and translates nothing inside an image. With it enabled (`--ocr`, or the GUI checkbox), every image region is recognized, translated a paragraph at a time, and drawn back over the lines it read; the backing covers the box of each line that was read, never the paragraph around them and never the artwork, and each box is painted the colour sampled underneath it rather than white, with an ink chosen light or dark so it stays legible on either; every paragraph of body text on a page is drawn at one size, the largest the tightest of them can hold, while a heading or a caption keeps its own, and a translation that comes back identical to what was read is not drawn at all, because covering the picture buys nothing, and a translation that will not fit its box even at half the size it was read at is left as pixels and reported.
